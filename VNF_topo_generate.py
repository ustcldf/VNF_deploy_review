...
class fat_tree(object):
    ...
    def instant_topo(self, ):
        ...
        """
        #agg switch
        agg_switch_id = self.K+self.K*self.K/2
        for j in range(core_switch_id, agg_switch_id):
            agg_switch = VNF_switch()
            agg_switch.id = j
            self.switch[j] = agg_switch
            self.agg_switch_dict[j] = agg_switch
            #self.edge_switch_dict[j] = edge_switch
        #edge switch
        edge_switch_id = self.K+self.K*self.K
        for l in range(agg_switch_id, edge_switch_id):
            edge_switch = VNF_switch()
            edge_switch.id = l
            self.switch[l] = edge_switch
            self.edge_switch_dict[l] = edge_switch
        """
        ...
        #establish links
        """
        for c_s in self.core_switch_dict.iterkeys():
            for a_s in self.agg_switch_dict.iterkeys():
                self.core_link[c_s][a_s] = core_band
                self.core_link[a_s][c_s] = core_band
        """
        #root links
