...
class VNF_aggregation(object):
    ...
    def construct_d_mtr(self, request_seq_dict):                  #establish the d matrix
        ...
        REQUEST_NUM = len(request_seq_dict.keys())
            ...
            if req_tuple == None:
                while (len(req_lib_backup)):        #in case there is one request lefted
                    ...
                    if self.check_end(path, core_link, req_lib_backup[key], host_lib[host_id]):
                        ...
                        host_id = PowerNets_C.PowerNets(route, core_link, e_link, key, request_lib[key], host_lib, host_id)
                        req_lib_backup.pop(key)
                ...
                if self.check_end(path, core_link, request_lib[req_tuple[0]], host_lib[host_id]):
                    ...
                    host_id = PowerNets_C.PowerNets(route, core_link, e_link, req_tuple[0], request_lib[req_tuple[0]], host_lib, host_id)
                    req_lib_backup.pop(req_tuple[0])
                    self.modify_d_mtr(req_tuple[0],d_mtr)
                    if self.check_end(path, core_link, request_lib[req_tuple[1]], host_lib[host_id]):
                        host_lib[host_id].req_container.append(req_tuple[1])
                        ...
                        host_id = PowerNets_C.PowerNets(route, core_link, e_link, req_tuple[1], request_lib[req_tuple[1]], host_lib, host_id)
                        self.modify_d_mtr(req_tuple[1],d_mtr)
                        req_lib_backup.pop(req_tuple[1])
                        continue
                    ...
                    for m in host_2r_d.keys():
                        if m in host_candinate:    #the id must in host_candinate
                            if m == i:
                                ...
                                factor_dict[m] = alpha * host_2r_d[m]/(host_2r_dt+EPSILON_F) + beta * VNF_to_match[m][0]/(VNF_2m_bt+EPSILON_F)               #it is a balance between d and aver traffic volume
...
def VNF_simulation_cosin(data_back):
    ...
    with open('F:/VNF_Review/ldf/simulation/data_store/host_lib_cosin.pickle', 'w') as f:
    ...
    with open('F:/VNF_Review/ldf/simulation/data_store/saved_info_dict_cosin.json', 'w') as f:
