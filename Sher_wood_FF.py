...
def VNF_simulation_SherWood_FF_BF(data_back):
    fattree = fat_tree()
    fattree.K = 16
    #fattree.__init__()
    ...
    print "FF&BF SherWood"
    
    req_sequence_list = []
    for i in range(len(NFR_lib_ff)):
        req_sequence_list.append(i)
    
    random.shuffle(req_sequence_list)
    #host_id = 128
    host_info_ff_min = req_2_host.FF_map(route, core_link_ff, e_link_ff, NFR_lib_ff, host_lib_ff, req_sequence_list)
    host_info_bf_min = req_2_host.BF_map(route, core_link_bf, e_link_bf, NFR_lib_bf, host_lib_bf, req_sequence_list)
    shuffle_times = 1000
    host_num_dict_ff_sher = {}
    host_num_dict_bf_sher = {}
    for j in range(shuffle_times):
        ...
        if host_info_ff[0]+1 in host_num_dict_ff_sher.keys():
            host_num_dict_ff_sher[host_info_ff[0]+1] += 1
        else:
            host_num_dict_ff_sher[host_info_ff[0]+1] = 1
        
        if host_info_bf[0]+1 in host_num_dict_bf_sher.keys():
            host_num_dict_bf_sher[host_info_bf[0]+1] += 1
        else:
            host_num_dict_bf_sher[host_info_bf[0]+1] = 1
        
        if host_info_ff[0] <= host_info_ff_min[0]:
            host_info_ff_min = host_info_ff
        if host_info_bf[0] <= host_info_bf_min[0]:
            host_info_bf_min = host_info_bf
    print "host_num_dict_ff_sher", host_num_dict_ff_sher
    print "host_num_dict_bf_sher", host_num_dict_bf_sher
    print "*********start save info*********"
    
    host_id_ff = host_info_ff_min[0]
    host_lib_ff = host_info_ff_min[1]
    ...
    with open('F:/VNF_Review/ldf/simulation/data_store/saved_info_dict_FF_Sher.json', 'w') as f:
    ...
    host_id_bf = host_info_bf_min[0]
    host_lib_bf = host_info_bf_min[1]
    ...
    with open('F:/VNF_Review/ldf/simulation/data_store/saved_info_dict_BF_Sher.json', 'w') as f:
