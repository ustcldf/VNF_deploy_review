...
def VNF_simulation_T_SAT_PN(data_back):
    fattree = fat_tree()
    fattree.K = 16
    #fattree.__init__()
    ...
    print "*****start to map the requests to hosts*****"
    host_id = req_2_host.PowerNets(route, core_link_PN, e_link_PN, NFR_lib_PN, host_lib_PN, VNF_move_id_list)
    print "*****start pod adjust*****"
    host_lib_PN = adjust_pod.pod_adjust(host_id, fattree.K, host_lib_PN, request_lib_PN)
    
    print "after pod_adjust, the saved num: "
    pod_adjust_saved_num = adjust_pod.pod_adjust_saved(host_id,host_lib_PN)
    
    print "*****start adjust whole*****"
    host_info = adjust_whole.mignate_request_all(route, core_link, e_link, request_lib_PN, host_id, host_lib_PN)
    saved_host = host_info[0]
    host_lib_store = host_info[1]
    ...
    with open('F:/VNF_Review/ldf/simulation/data_store/host_lib_T_SAT_PN.pickle', 'w') as f:
        pickle.dump(host_lib_store,f)
    saved_info_dict = {}
    ...
    with open('F:/VNF_Review/ldf/simulation/data_store/saved_info_dict_T_SAT_PN.json', 'w') as f:
        json.dump(saved_info_dict,f)
    #saved_host = []
    #host_resource_left_check(host_lib_PN,host_id,saved_host)
